...
$$
t_{CLA}=t_{pg}+t_{pg\_block}+(\frac{N}{k}-1)t_{AND \_ OR}+kt_{FA}
$$

表达式详解：

* $t_{pg}$：表示每个全加器生成产生信号与传播信号，这个过程只会用到一个逻辑门。
* $t_{pg\_block}$:表示在$k$位快中的产生信号。这个过程会用到$2k$个逻辑门。
* $t_{AND\_OR}$：表示单个快产生进位信号的时间，此时这个需要$2$个逻辑门。从最低位块，向最高位块需要传播的$\frac{N}{k} - 1$，减一表示最高位块，不急着去算进位信号。
